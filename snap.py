...
def evaluate(model, _estimator, dataloader, stage, max_loss):
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        logits = 0
        loss = 0
        ensemble_size = 1 if stage != "Test" else 30
        for i in _estimator:
            i.eval()
...
if training:
    ...
    for epoch in range(5):  # loop over the dataset multiple times
        ...
        if counter % n_iters_per_estimator == 0:
            ...
            snapshot = transformers.PerceiverForImageClassificationFourier.from_pretrained("deepmind/vision-perceiver-fourier",
                                                                     num_labels=num_labels,
                                                                     id2label=id2label,
                                                                     label2id=label2id,
                                                                     ignore_mismatched_sizes=True)
            ...
            estimators_.append(snapshot)

        model, loss_all, count_batch, max_loss = evaluate(model, estimators_, val_dataloader, "Val", max_loss)
